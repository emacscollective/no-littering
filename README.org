...
visited file.  To avoid littering these backups all over the place,
you may wish to collect /most/ of them in a single directory:
...
With this value, most files are backed up in the ~var~ directory, but
for files in ~/tmp~ and ~/dev/shm~, the backups are created in the same
directory as the visited file.  This helps reduce the risk of leaking
secrets; in case an command-line tools has the user edit a secret by
opening a clear-text file using ~emacs~ / ~emacsclient~.  For more
information about this risk, see [[https://www.reddit.com/r/emacs/comments/12tj72z][this explanation]].

**** Auto-save files

By default auto-save files are created in the same directory as the
visited file, except for remote files, which are created in ~/tmp~ on
the local machine.  To collect /most/ auto-save files in a single
...
The first element preserves the default behavior for remote files.
The second element tries to keep auto-save files for visited files,
which exist in in-memory file-systems, on the same file-system.  The
third element places all other auto-save files in the ~var~ directory.
Also see the previous section about the risk of leaking secrets.
